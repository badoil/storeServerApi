...
module.exports.getList = async (options) => {
    try{
        ...
        if(options){
            if(options.deal_idx) {
                sql += ` WHERE deal_idx = ${options.deal_idx}`
            } else if(options.repr_cat_id){
                sql += ` WHERE repr_cat_id = ${options.repr_cat_id}`
            } else if (options.deal_id){
                sql += ` WHERE deal_id = ${options.deal_id}`
            }
...
module.exports.subJoinGet = async(options) => {
    try{
        ...
        if(ac_tgt_goods_or_not && mark){
            columnClause += ` AND deal.ac_tgt_goods_or_not = '${ac_tgt_goods_or_not}'`
        ...
        if(!srch_orderby && mark){
            srch_orderby = 'c.first_create_dt_time'
        }
        if(!srch_seq){
            srch_seq = 'asc'
        }
        ...
        let sql = `SELECT c.*, deal.deal_id  
                    ...
                    ${limitClause}
                    `
        let sql2 = `SELECT c.*, goods.suppl_code, goods.goods_id 
                    FROM (
                        SELECT deal_id
                        FROM deal
                        WHERE 1=1
...
module.exports.subJoinDealGet = async(options) => {
    try{
        ...
        let sql = `SELECT c.*, deal.*
                    ...
                    ${limitClause}
                    `
        // let sql2 = `SELECT c.*, goods.goods_id
        //             FROM (
        //                 SELECT *
        //                 FROM deal
        //                 WHERE 1=1
