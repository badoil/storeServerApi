  ...
  module.exports.getList = async(req, res, next) => {
    try{
      ...
      const aggregationsFunc = async(options, values, column) => {
          ...
          // 중간값 : 만원 이하 / 범위 : 5천원 이하, 5천원~만원, 만원~만오천원, 만오천원 이상
          // 중간 값 : 만원 이상 5 만원 이하 / 범위 : 만원 이하, 만원~2만오천원, 2만 오천원~5만원, 5만원 이상
          // 중간 값 : 5만원 이상 / 범위 : 3만원 이하, 3~6만원, 6~9만원, 9만원 이상 
      ...
      if(options.exp_cat_idx){
        ...
        if((options.minPrice && options.maxPrice) || options.ac_tgt_goods_or_not || options.free_deliv_or_not){
          ...
          // let searchResult = await handler.getSearch(options)
          // //console.log("searchResult:", searchResult)
          // if(searchResult.length === 0){
          //   throw{ status: 404, errorMessage: "No price or after care goods or free deliv result under these conditions"};
          // }
          // let tempDealIdArray = searchResult.map(a => a.deal_id)
          // dealIdArray.push(tempDealIdArray)
        }
        ...
        if(label === '늘 사던 것'){
          ...
          for(let i=0; i<3; i++){
            ...
            //catList.push({ name: reprCatIdResult[0].cat_name, list: result })
          ...
          randomResult.push({name: label, alwaysBuyGoods})
        } 
        let object = {
          name : label,
          list : randomResult
...
//     const cartResult = await cartHandler.getList({cust_idx: params.cust_idx})
//     const dealIds = cartResult.map(cart => cart.deal_id)

//     const goodsResults = await goodsHandler.getList({goods_id: dealIds})
//     console.log('goodsResults:',goodsResults)
//     const reprCatIds = goodsResults.map(good => good.repr_cat_id)
//     const catResults = await representativeCategoryHandler.getList({cat_id: reprCatIds})
//     const catList = catResults.map(cat => cat.cat_name)

//     res.status(200).json({result: catList})
