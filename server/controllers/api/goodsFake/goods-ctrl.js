...
const handler = require('./goodsFake-handler')
...
module.exports.getList = async (req, res, next) => {
  try {
    ...
    if(params.type!=undefined){
      const sortTypeIdx = await goodsSortTypeHandler.getList({type:params.type})
      console.log('sortTypeIdx : ', sortTypeIdx[0].IDX)
      result = await handler.getList({sortTypeIdx:sortTypeIdx[0].IDX})
    } else {
